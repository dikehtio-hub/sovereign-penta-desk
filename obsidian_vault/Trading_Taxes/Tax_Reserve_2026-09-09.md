# 🏦 Tax Escrow & Safe Bankroll Report (2026-09-09)

> Generated automatically by Tax Reserve Agent.
> Desk: [[Desk_05_Tax_Reserve_Agent|Desk 5: Tax Reserve Agent]] · Shell twin: `python -m Tax_Reserve_Agent.main export`
> Last updated: `2026-09-09 23:59:46`

---

### 📊 Portfolio & Bankroll Summary
- **Total Liquid Balance:** `$0.00`
- **🛡️ Tax Escrow (DO NOT SPEND):** `$0.00`
- **🚀 Safe Deployable Bankroll:** **`$0.00`**
- **Escrow Ratio:** `0.0%`

**Escrow composition**
- Short-term capital gains: `$0.00`
- Long-term capital gains: `$0.00`
- CME futures (IRC §1256, 60/40 @ `25.8%`): `$0.00`
- Ordinary income (IRC §61): `$0.00`
- Gambling / wagering (IRC §165(d)): `$0.00`

---

### 📈 Realized Capital Gains (Tax Year 2026)
- **Net Capital Gains:** `$0.00`
- **Gross Gains:** `+$0.00`
- **Gross Losses:** `-$0.00`
- **Composite Tax Rate:** `32.4%`

### 💰 Ordinary Income (IRC §61 - Funding Yields / Staking)
- **Net Ordinary Income:** `$0.00`
- **Gross Income:** `+$0.00`
- **Gross Expense:** `-$0.00`


### 📂 Asset Breakdown
- *No closed trades yet.*

---

### 🎯 Strategy Capital Buckets (Allocations off Safe Bankroll)
- **`consensus_copy`:** `20%` ($0.00)
- **`dutched_arb`:** `25%` ($0.00)
- **`hl_basis_harvest`:** `25%` ($0.00)
- **`hl_liquidation_fade`:** `5%` ($0.00)
- **`sandbox`:** `10%` ($0.00)
- **`sports_betting`:** `15%` ($0.00)


---
*Note: This is an internal managerial estimate to prevent bankroll over-allocation across all 4 desks.*
