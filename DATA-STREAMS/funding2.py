    ...
    while True: # Loop forever to reconnect if the connection drops
        try:
            async with connect(websocket_url) as websocket:
                # This print statement will only show up if you run from a persistent terminal
                # print(f"Connected to {symbol} stream.") 
                while True:
                    message = await websocket.recv()
                    data = json.loads(message)

                    # The calculation is now saved to a variable.
...
if __name__ == "__main__":
    # --- FIX: ADDED A SAFETY NET TO CATCH ERRORS AND PAUSE ---
    try:
        # Try to run the main program
        asyncio.run(main())
    except Exception as e:
        # If any error happens, print it to the screen
        print(f"\n--- AN ERROR OCCURRED ---\n")
        print(e)
    finally:
        # No matter what, pause at the end and wait for the user
