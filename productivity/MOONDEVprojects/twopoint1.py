...
def load_tasks():
    with open(r'C:\Users\ixis1\Desktop\DEV\productivity\MOONDEVprojects\tasks.json', 'r') as f:
        tasks = json.load(f)
    return tasks 
        ...
        for index, (task, s_time, e_time) in enumerate(schedule):
            if index < current_index:
                ...
                if remaining_minutes < 2:
                    ...
                    cprint(f'{task} - {remaining_minutes} mins', 'white', 'on_green')
            else:
                print(f'{task} @ {s_time.strftime("%H:%M")}')


        list_of_reminders = [
            "Ihave a 1000 percent algo",
            "time is irrelevant, keep swimin",
            "every day i get better",
            "rest at the end",
            "jobs not finished", 
            "deal was already made",
            "I'm the best algo trader in the world",
        ]
