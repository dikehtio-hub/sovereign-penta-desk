#Moondev D.4
#use a ccxt exchange like phemex, bybit, or a dex
...
secret_key = d.private_key #grab from metamask or whatever wallet?
...
#MAKE A BUY AND SELL ORDER
def limit_order(coin, is_buy, limit_px, reduce_only, account):
    ...
    if is_buy == True:
        print(f"limit BUY ordder placed thanks moon dev, resting: {order_result['response']['data']['statuses'][0]}")
    else:
        print(f"limit SELL ordder placed thanks moon dev, resting: {order_result['response']['data']['statuses'][0]}")
    
    return order_result

coin = symbol
is_buy = True # False =  Sell
limit_px = ask_bid(coin)
reduce_only =  False
secret_key = d.private_key
account = LocalAccount =  eth_account.Account.from_key(secret_key)

#buy order
limit_order(coin, is_buy, size, limit_px, reduce_only, account)
